...
struct Add_Report_Model : Codable {
    let status : Bool?
    let code : String?
    let data : String?
...
struct BankModel : Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        status = try values.decodeIfPresent(Bool.self, forKey: .status)
        code = try values.decodeIfPresent(String.self, forKey: .code)
        data = try values.decodeIfPresent(BankData.self, forKey: .data)
    }
...
struct BankData : Codable {
    ...
    let bank_account_user_name : String?
    let bank_account_number : String?
    let bank_account_iban : String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case bank_account_user_name = "bank_account_user_name"
    ...
    init(from decoder: Decoder) throws {
        ...
        bank_account_user_name = try values.decodeIfPresent(String.self, forKey: .bank_account_user_name)
        bank_account_number = try values.decodeIfPresent(String.self, forKey: .bank_account_number)
        bank_account_iban = try values.decodeIfPresent(String.self, forKey: .bank_account_iban)
    }
